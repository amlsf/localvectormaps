...
In terms of granularity of detail, I felt that either the zipcode or neighborhood level would be granular enough without being overwhelming and ultimately settled on zipcodes as neighborhood boundaries were less clear cut. I chose  3 metrics: sales price for those with a specific budget in mind, sales price per square foot to normalize for an apples-to-apples comparison, and price % change between any two years of the user's choosing between 2006-2013 to visualize growth and recovery. 
...
Another major task was designing the structure of the database and how I wanted to store all the longlat data so it could be easily accessed without unnecessary loops. A good amount of time was spent figuring out how to set up the database and write a script to use a Ray Casting algorithm to determine the membership of each home in its respective region, be it block group, census tract or subcounty.  
...
By 2009-2010, real estate home values had reached bottom compared to the peak in 2006. This is apparent from the warm colors indicating home value loss covering the map when you select 2006 as the base year and 2009 as the comparison year. 
![Loss] (/screenshots/3-2006-2009.JPG)

Recently, real estate values have been recovering as you can see from the overwhelming green when selecting 2011-2013 on the range slider. Growth is especially strong near San Francisco, around Palo Alto, and Monterrey. 
![Growth] (/screenshots/4-2011-2013.JPG)

By comparing each of the years of 2010-2013 to the baseline of the 2006 peak in home values to gauge whether prices have returned to previous highs, you'll notice a single, rapidly growing green hot spot centered around Mountain View and Palo Alto. THis indicates those regions have median home values that are now exceeding 2006 highs and are growing at a faster pace than the areas around San Francisco and Monterrey, which have not yet returned to previous highs. 
...
I plan to add options to view additional cuts of the data, including the ability to filter by number of bedrooms, baths, square footage and price level both for the heatmap and the active listing markers. In addition, the users will be able to view the choropleth map with regions broken out by increasing granularity, such as block groups. 

I realized from the project this is much more complex than it seems both in terms of managing how to structure my code and managing performance. As I noted earlier, as the number of permutations of options that a user can select grows, the less feasible it is to pre-calculate and store metrics for every scenario in the database. However, having the server run many calculations on the fly can significantly slow down the application. 
...
I'd like to create interactive time series line graphs that allow users to select multiple metrics and regions to compare on the same graph and charts. 
