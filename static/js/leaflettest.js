...
// update map based on form  
// as extra optimization, memo-ization: might stash away last parameters sent in global environment, when gets called, check whether new request is same as current parameters
// function updateMapfromForm() {

  // if toggleActivelistings: (user hits the button)
    // if not active listings is showing: (check listings are not already showing)
  //     showactivelistings():
  //   else 
  //     hide actinglistings
  ...
  //   ...  // other params setup
  // else if zipcode:
  //   params[granularity] = 'code'
  //   ...

  // # of bedrooms (may not need if else, because can assign  button to value and just extract value)
  // if 2BR selecd: 
  //   params[BR] = 2


  // if #SP checked, then do sthg
        // updatemap(... pass parameter to show Sales Price, etc.)  set up parameters to take a dictionary of items
        // updatelegend(...pass correct paratmers to show proper legend)
  // else if SPS, then do
        // updatemap() 
        // updatelegend()
  // else if SPSC, then do
  //     extract value of range slider
        // updatemap() 
        // updatelegend()
// }


// change form itself based on form
// function updateForm() {
//   if toggleoff heatmap:
//     hide metric options
//   else:   
//     show metric optioins
// }


// ON THE BACKEND: 
// Assembling a query dynamically: 
// sending back: JSON Stringify
// Assemble query by doing if-else statement with concatenation usign "Where = ??" clause and insert that params into it 
// If Params != None:
//   then 
// concatenate SQL query by adding new where clause  with ?? and inserting the parameter
